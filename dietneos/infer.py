...
def expected_CLs(model_maker, solver_kwargs):
    ...
    def get_expected_CLs(yields, test_mu, pvalues=["CLs"]):
        ...
        # the constrained fit
        numerator = (
            to_bounded_vec(c_fitter(initval, [yields, test_mu]), bounds)
            if transforms
            else c_fitter(initval, [yields,test_mu])
        )
